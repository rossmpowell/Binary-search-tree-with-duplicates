...
public class Book implements Comparable<Book> {
    ...
    */
    ...
    public int compareTo(Book theBook) throws NullPointerException {
    	if(this.getTitle().compareTo(theBook.getTitle()) == 0) {
    		if(this.getAuthor().compareTo(theBook.getAuthor()) == 0) {
    			if(this.getEdition() == theBook.getEdition()) {
    	    		return 0;
    	    	} else if (this.getEdition() > theBook.getEdition()) {
            		return 1;
            	} else if (this.getEdition() < theBook.getEdition()) {
            		return -1;
            	} else {
            		throw new NullPointerException("Cannot compare Book object to null");
            	}
        	} else if (this.getAuthor().compareTo(theBook.getAuthor()) > 0) {
        		return 1;
        	} else if (this.getAuthor().compareTo(theBook.getAuthor()) < 0) {
        		return -1;
        	} else {
    	...
    	} else if (this.getTitle().compareTo(theBook.getTitle()) > 0) {
    		return 1;
    	} else if (this.getTitle().compareTo(theBook.getTitle()) < 0){
    		return -1;
    	} else {
